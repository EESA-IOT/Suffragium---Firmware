...
bool Sodaq_RN2483::setMacParam(const char* paramName, const uint8_t* paramValue, uint16_t size)
{
	...
	if(paramName == STR_CLASS)
	{
		this->loraStream->print(static_cast<char>(paramValue[0]));
	}
	else
		{
...
uint8_t Sodaq_RN2483::onMacRX()
{
	...
	debugPrintLn("this->packetReceived");
	
	debugPrintLn(token);
	debugPrintLn(this->packetReceived);
	debugPrintLn(this->receivedPayloadBuffer);
	debugPrintLn("finish this->packetReceived");
    return NoError;
}
